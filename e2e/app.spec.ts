...
let window: Page
const screenshotPath = 'e2e/report/screenshots/'
...
test.describe('Dashboard test', () => {
  test('add attendance', async () => {
    ...
    await window.click('#' + KEYWORDS.dashboard)
    // choose employee
    await window.selectOption('select#emps', 'John Doe')
    await window.click(`text=Add ${KEYWORDS.attendance}`)
  })
})
